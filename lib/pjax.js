  ...
  Pjax = (function(_super) {
    ...
    Pjax.prototype._init = function() {
      ...
      if (this.opts.history) {
        this.on('pushstate.pjax', function(e, state) {
          return history.pushState(state, state.name, state.url);
        });
        this.on('replacestate.pjax', function(e, state) {
          return history.replaceState(state, state.name, state.url);
        });
    ...
    Pjax.prototype.load = function(url, opts) {
      ...
      if (simple.url().toString() === url.toString()) {
        return;
      }
      opts = $.extend({
        nocache: false
      ...
      this.trigger('pushstate.pjax', [page]);
    ...
    Pjax.prototype.loadPage = function(page) {
      ...
      this.setCache(page);
      this.trigger('replacestate.pjax', [page]);
      pageId = $page.attr('id');
    ...
    Pjax.prototype.setCache = function(page) {
      ...
      Pjax.pageCache[this.url.toString()] = page;
      return page;
    };

    Pjax.prototype.getCache = function() {
      return Pjax.pageCache[this.url.toString()];
    };

    Pjax.prototype.clearCache = function() {
      return Pjax.pageCache[this.url.toString()] = void 0;
    };
