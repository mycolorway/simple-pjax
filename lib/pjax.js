(function() {
  ...
  Pjax = (function(_super) {
    ...
    Pjax.prototype._init = function() {
      ...
      this.on('pjaxbeforeload', function(e, page) {
        if (page.params && page.params.scrollPosition) {
          return $(document).scrollTop(page.params.scrollPosition.top).scrollLeft(page.params.scrollPosition.left);
        } else {
          return $(document).scrollTop(0).scrollLeft(0);
        }
      });
      ...
      if (this.opts.history) {
        $(window).off('popstate.pjax').on('popstate.pjax', function(e) {
          ...
          document.title = _this.opts.title.replace('{{ name }}', state.name);
      ...
      if (this.opts.autoload) {
        if (history.state) {
          ...
          document.title = this.opts.title.replace('{{ name }}', history.state.name);
        }
    ...
    Pjax.prototype.load = function(url, opts) {
      ...
      document.title = this.opts.title.replace('{{ name }}', state.name);
